...
## Source-pinned 77.2 maintenance line

This change is based on package tag `77.2.1` / Uno commit
`27a8963ded97286e987b51c942ebd6fa937e89bf`. It retains ICU **77.1**,
Emscripten **3.1.56**, the Windows static-CRT patch, macOS/iOS configure options,
the shim, all four WASM thread/SIMD variants and the existing 62-locale data
filter. It does not import unrelated platforms from newer `main`.
...
`version.json` selects a **new prerelease** on the `77.2.2-provenance` line.
NBGV determines the precise version in CI. `eng/Pack.ps1` rejects old 77.2.1
identities and existing output directories. Never repack/restamp an old archive
or use an old package's native files as replacement build outputs.

### License ownership

Each of the four packages declares a NuGet file license, `LICENSE.txt`.
...
One WASM archive (repeat all four true/false combinations; do not omit any):
...
`unoicu.a/3.1.56/{st,mt,st,simd,mt,simd}/unoicu.a` (the last two names each
...
`lipo`, and Xcode/iOS SDKs for the normal iOS cross-builds. No platform is
