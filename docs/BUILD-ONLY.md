...
or a GA gate. No old package bytes are inputs. A successful local unit test is
...
manual authorize + exact SHA + contracts -> source archive/notices/inventory
  -> data (Linux Docker) -------------------------------+
  -> wasm [st, mt, st,simd, mt,simd] (Linux Docker) ------+
...
3.1.56 and all four thread/SIMD variants, at most two concurrently.
...
python3 eng/build_only.py build --kind wasm --variant st
python3 eng/build_only.py build --kind wasm --variant mt
python3 eng/build_only.py build --kind wasm --variant st,simd
python3 eng/build_only.py build --kind wasm --variant mt,simd
...
* macOS Intel and ARM64 hosted capacity, Xcode/command-line tools, `unzip`,
  Python 3, make, clang, `lipo`, `otool`. iOS SDKs are not needed for this path.
* Budget for 10 native/merge row jobs plus authorization and aggregate staging,
...
* row/variant, Uno commit and unchanged ICU source/archive/license lock;
...
3.1.56 manifest. Only small registry metadata was read to resolve these pins;
...
bundles (including notices/logs/manifests) and an exact 11-file `payload` tree:
...
payload/nuget/uno.icu-wasm/buildTransitive/native/unoicu.a/3.1.56/
  {st,mt,st,simd,mt,simd}/unoicu.a
```

The last two WASM directory names each include a comma. These are the existing
package input shapes, **not nupkgs**. No iOS placeholders are created and
`eng/Pack.ps1` still requires the full four-package release matrix. Any future
