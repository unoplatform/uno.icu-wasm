...
def expected_payloads(kind, variant=""):
    if kind in VARIANTS and variant not in VARIANTS[kind]:
        raise ValueError("Unexpected matrix variant")
    if kind == "data":
        return {"nuget/icudt.dat"}
    if kind == "wasm":
        return {f"nuget/uno.icu-wasm/buildTransitive/native/unoicu.a/3.1.56/{variant}/unoicu.a"}
    if kind == "windows":
    ...
    if kind == "staging":
        return set().union(expected_payloads("data"), expected_payloads("macos-universal"),
                           ...
                           *(expected_payloads("wasm", v) for v in VARIANTS["wasm"]))
    raise ValueError("Unknown build row")


def validate_payloads(bundle, kind, variant):
    actual = file_hashes(bundle / "payload")
    if set(actual) != expected_payloads(kind, variant):
...
class Build:
    def __init__(self, kind, variant=""):
        ...
        self.directory = OUT / (kind + ("-" + variant if variant else ""))
    ...
    def finish(self, inputs=None):
        check_authorization()
        validate_payloads(self.directory, self.kind, self.variant)
        manifest = {
            ...
            "unverifiedRows": ["ios", "iossim", "WASM runtime", "Windows ARM64 runtime"],
        }
...
def verify_bundle(directory, kind, variant="", env=None):
    ...
    validate_payloads(directory, kind, variant)
...
def docker_build(build, archive):
    ...
    if kind == "wasm":
        args += ["--build-arg", "EMSCRIPTEN_VERSION=3.1.56",
    ...
    build.payload(output / name, next(iter(expected_payloads(kind, variant))))
...
def assemble(incoming):
    ...
    rows += [(kind, v) for kind in ("wasm", "windows", "macos") for v in VARIANTS[kind]]
    expected = {"build-only-" + kind + ("-" + v if v else "") for kind, v in rows}
    ...
    for kind, variant in rows:
        name = "build-only-" + kind + ("-" + variant if variant else "")
        directory = incoming / name
        parents[name] = verify_bundle(directory, kind, variant)
        ...
        if kind not in ("source", "macos"):
            for payload in expected_payloads(kind, variant):
...
def main():
    ...
    if args.command == "source":
        ...
        expected_payloads(args.kind, args.variant)
        ...
        build = Build(args.kind, args.variant)
